...
// Snapshot is one reading of a set of command surfaces, kept so a later reading
// can be subtracted from it.
//
// Where the snapshots live is the caller's to decide: every function here takes
// the directory rather than resolving one, so a consumer's data directory never
...
//
// The timestamp is passed in rather than read from the clock, so a caller can
// write a snapshot with a time it controls and a test does not have to freeze
// anything.
func Save(dir string, tools []*Tool, taken time.Time) (*Snapshot, error) {
	...
	snap := &Snapshot{Version: next, Taken: taken, Tools: tools}
