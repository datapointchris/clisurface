...
// node and tool build surfaces by hand, so a diff test states the shape it is
// about rather than the help screens that would have produced it.
...
func TestSaveAssignsTheNextVersion(t *testing.T) {
	...
	first, err := Save(dir, []*Tool{tool("demo")}, testTime)
	if err != nil {
		t.Fatalf("save: %v", err)
	}
	second, err := Save(dir, []*Tool{tool("demo")}, testTime)
	if err != nil {
		t.Fatalf("save: %v", err)
	}
...
func TestSavedSurfaceLoadsBackTheSame(t *testing.T) {
	...
	saved, err := Save(dir, tools, testTime)
	if err != nil {
		t.Fatalf("save: %v", err)
	}
...
func TestVersionsIgnoresWhatIsNotAVersion(t *testing.T) {
	dir := t.TempDir()
	if _, err := Save(dir, []*Tool{tool("demo")}, testTime); err != nil {
		t.Fatalf("save: %v", err)
	}
